...
/******************************** PROTOTYPES *********************************/
// mailingList node struct typedef
typedef struct {
    int proc_ID;
    int flag;
    char mailbox_ID[__MAX_MAILBOXID_LENGTH];
} listNode;
...
static const int sizeOfListNode = sizeof(listNode);

// list of sender/reciever info: mailingList prototype
static listNode * mailingList;


// helper functions: mailingList searches
static int findMailboxID (const char *, listNode *);
static int findProcID (pid_t, listNode *);
...
{ 
    //kmalloc will only be called IFF (flag == (__FLAG_OPEN | __FLAG_SEND) && !findMailboxID() && !findProcID()) 
    //if(above){ call kmalloc(); }
    //(above) should fail and return error if: 
    
    //create mailingList node with {mailboxID,procID,flag};
    //add node to mailingList;
    ...
    printk(KERN_INFO "mailboxID: %s\n", mailboxID);
    printk(KERN_INFO "procID: %i\n", procID);
	printk(KERN_INFO "kernBuffCapacity: %i\n", kernBuffCapacity);
...
static unsigned long ourmb_recv(const char * mailboxID, pid_t procID, char * recvBuff, char * kernBuff)
{
    ...
    printk(KERN_INFO "mailboxID: %s\n", mailboxID);
    printk(KERN_INFO "procID: %i\n",    procID);
	printk(KERN_INFO "recvBuff: %p\n",  recvBuff);
    printk(KERN_INFO "kernBuff: %p\n",  kernBuff);
	return 0;
}
...
{	//Hijack 4 unimplemented system calls
    ...
    //same as above
        listNode * mailingList = kmalloc(__MAX_READERS_WRITER * sizeOfListNode ,GFP_ATOMIC);
        printk(KERN_INFO "kmalloc() allocated: %zu bytes of memory for kernBuff\n", ksize(kernBuff));
        printk(KERN_INFO "kmalloc() allocated: %zu bytes of memory for mailingList\n", ksize(mailingList));
